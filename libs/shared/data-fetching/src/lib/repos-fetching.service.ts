...
import {
  GithubRepoDTO,
  GithubSerializedSearch,
  GitlabRepoDTO,
  GitlabSerializedRepo,
  SlideData,
} from './model';

export const REPO_TOPIC = 'ge-showroom';

export const GH_HOST = 'https://github.com';
export const GH_API = 'https://api.github.com';
export const GH_USER = '4javier';
export const GH_API_VERSION = '2022-11-28';

export const GL_HOST = 'https://gitlab.com';
export const GL_API = 'https://gitlab.com/api';
export const GL_USER = 'gianpiero.errigo';
export const GL_API_VERSION = 'v4';
...
export class ReposFetchingService {
  ...
  constructor(private http: HttpClient) {
    this.githubRepos$ = http
      .get<GithubSerializedSearch>(`${GH_API}/search/repositories`, {
      ...
      .pipe(
        ...
        catchError(() =>
          throwError(
            () =>
              new Error('An error occurred while fetching repos from Github')
          )
        ),
    ...
    this.gitlabRepos$ = http
      .get<Array<GitlabSerializedRepo>>(
      ...
      .pipe(
        ...
        catchError(() =>
          throwError(
            () =>
              new Error('An error occurred while fetching repos from Gitlab')
          )
        ),
        concatAll(),
    ...
    const mapRepoToSlide = (
      repo: GithubRepoDTO | GitlabRepoDTO
      ...
      return this.http.get<{ content: string }>(repo.imageUrl).pipe(
        ...
        map((imageJson) => ({
          ...
          link:
            repo.origin === 'github'
              ? `${GH_HOST}/${GH_USER}/${repo.name}`
              : repo.origin === 'gitlab'
              ? `${GL_HOST}/${GL_USER}/${repo.name}`
              : 'no_link',
          origin:
            repo.origin === 'github'
              ? 'assets/github-badge.svg'
              : repo.origin === 'gitlab'
              ? 'assets/gitlab-badge.svg'
              : 'no_logo',
        }))
      );
    };
